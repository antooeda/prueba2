...
public class RecetaController {
    ...
    public String verRecetasPaciente( HttpServletRequest request,@PathVariable("idPaciente") int idPaciente,Model model){
    ...
    public String verRecetasEmitidas(HttpServletRequest request,Model model){
        Credencial c = (Credencial) request.getSession().getAttribute("medicoLogueado");
        if(c ==null){
            return "redirect:/login";
        }else{
    ...
    public String muestraFormReceta(@PathVariable("idPaciente") int idPaciente,Model model, HttpServletRequest request){
        Credencial c = (Credencial) request.getSession().getAttribute("medicoLogueado");
        if(c==null){
            return "redirect:/login";
        }else{
    ...
    public String muestraFormReceta(Model model,@PathVariable("idPaciente") int idPaciente,@ModelAttribute Receta r, HttpServletRequest request){
        Credencial c = (Credencial) request.getSession().getAttribute("medicoLogueado");
        int idMedico = c.getIdMedico().getId();
        if(c ==null){
            return "redirect:/login";
    ...
    public String eliminarReceta(@PathVariable("idReceta") int idReceta){
        Receta r = rDao.findById(idReceta);
